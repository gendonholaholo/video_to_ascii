...
for i in range(count):
    ...
    output = ascii_magic.from_image_file(s,
                                         columns=250,
                                         width_ratio=2,
                                         mode=ascii_magic.Modes.HTML)
    ascii_magic.to_html_file("html/frame" + str(i) + ".html",
                             output,
                             additional_styles='background:#222;')

path = r"/usr/bin/wkhtmltoimage"
config = imgkit.config(wkhtmltoimage=path)

for i in range(count):
    imgkit.from_file("html/frame" + str(i) + ".html",
...
video = cv2.VideoWriter("dah.mp4", fourcc, 30, (iw, ih))
